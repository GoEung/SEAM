...
def adaptive_min_pooling_loss(x):
    ...
    y = torch.topk(x.view(n, -1), k=3, dim=-1, largest=False)[0]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--weights", default='weights/ilsvrc-cls_rna-a1_cls1000_ep-0001.params', type=str)
    ...
    for ep in range(args.max_epoches):
        ...
        for iter, pack in enumerate(train_data_loader):
            ...
            label = label.cuda(non_blocking=False).unsqueeze(2).unsqueeze(3)
            ...
            loss_cls1 = criterion_cls(label1[:, 1:, :, :], label[:, 1:, :, :])
            loss_cls2 = criterion_cls(label2[:, 1:, :, :], label[:, 1:, :, :])
            ...
            loss_ecr1 = torch.mean(torch.topk(tensor_ecr1.view(ns, -1), k=3, dim=-1)[0])
            loss_ecr2 = torch.mean(torch.topk(tensor_ecr2.view(ns, -1), k=3, dim=-1)[0])
    ...
    torch.save(model.module.state_dict(), os.path.join('weights', args.session_name + time.strftime('%Y%m%d%H%M',
                                                                                                    time.localtime(
                                                                                                        time.time())) + '.pth'))
