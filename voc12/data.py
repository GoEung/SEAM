...
def load_image_label_from_xml(img_name, voc12_root):
    ...
    el_list = minidom.parse(os.path.join(voc12_root, 'annotation' ,img_name + '.xml')).getElementsByTagName('name')

    multi_cls_lab = np.zeros((3), np.float32)
...
def load_image_label_list_from_npy(img_name_list):
    ...
    cls_labels_dict = np.load('voc12/cls_labels_wsss.npy', allow_pickle=True).item()

    return [cls_labels_dict[img_name] for img_name in img_name_list]

def get_img_path(img_name, voc12_root):
    return os.path.join(voc12_root, 'images', img_name + '.jpg')

def load_img_name_list(dataset_path):

    img_gt_name_list = open(dataset_path).read().splitlines()
    img_name_list = [img_gt_name.split(' ')[0][:-4] for img_gt_name in img_gt_name_list]
